...
Form.prototype = {
	...
	toHTML: function(form){
		...
		if(this.submit){
			html = `${html}<div class="field"><input type="submit" value="${this.submit}" /></div>`
		}
		if(this.formWrapper){
		...
		function loop(obj, name){
			for(let i in obj){
				...
				if(field.fields){
					...
					if($this.errorAfterField && !('errorAfterField' in field)){
						console.log('default!')
						field.errorAfterField = $this.errorAfterField
					}
...
function createCallback($this, handler, type, req, res, next){
	...
	handler[type] = form => {
		...
		// If routing form
		if($this.postRender){
			if($this.clearOnSuccess && type === 'success'){
				res.locals[$this.local] = $this.toHTML()
			}
			else{
